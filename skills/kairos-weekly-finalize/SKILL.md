...
3. **Verify week shape** -- re-read ClickUp via `kairos/tools/clickup-today.sh` for Mon-Fri to confirm the shape matches what the user set up. If the frog, heaviest day, or score coverage changed based on the user's ClickUp moves, update the Week Shape section.
